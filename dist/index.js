...
var isEnvFileExists = (0, import_node_fs.existsSync)(".env");
if (!isEnvFileExists) {
...
var CustomHono = class extends import_zod_openapi.OpenAPIHono {
};
...
var logtail = env.LOGTAIL_TOKEN ? new import_node.Logtail(env.LOGTAIL_TOKEN, {}) : void 0;
var logEvent = (message, eventData, severity = "info") => {
  if (eventData) {
...
// src/lib/docs.ts
var import_hono_api_reference = require("@scalar/hono-api-reference");
...
var appModulesList = [
  {
    name: "workspaces",
    description: "App-specific context entity. Workspace functions for end-users to personalize how they interact with their projects and the content in each project. Only the creator has access and no other members are possible."
  },
  {
    name: "projects",
    description: "App-specific context entity. Projects - like organizations - can have multiple members and are the primary entity in relation to the content-related resources: tasks, labels and attachments. Because a project can be in multiple workspaces, a relations table is maintained."
  },
  {
    name: "tasks",
...
var commonModulesList = [
  {
    name: "me",
    description: "Current user endpoints. They are split from `users` due to a different authorization flow."
  },
  {
    name: "users",
  ...
  {
    name: "memberships",
    description: "Memberships are one-on-one relations between a user and a contextual entity, such as an organization. It contains a role and archived, muted status"
  },
  {
    name: "organizations",
    description: "Organizations - `organization` - are obviously a central `entity`."
  },
  {
    name: "requests",
    description: "Receive public requests such as contact form, newsletter and waitlist requests."
  },
  {
    name: "general",
    description: "Endpoints that overlap multiple entities or are meant to support the system in general."
  },
  {
    name: "auth",
    description: "Multiple authentication methods are included: email/password combination, OAuth with Github. Other Oauth providers and passkey support are work in progress."
  }
];
var docs = (app4) => {
...
var nanoid = (0, import_nanoid.customAlphabet)("abcdefghijklmnopqrstuvwxyz0123456789");

// src/middlewares/logger/logger.ts
var humanize = (times) => {
  const [delimiter, separator] = [",", "."];
  const orderTimes = times.map(
    (v) => v.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, `$1${delimiter}`)
  );
...
function log(fn, prefix, logId, method, path, status = 0, elapsed, user, org) {
  const out = prefix === "req" /* Incoming */ ? `${prefix} ${logId} ${method} ${path}` : `${prefix} ${logId} ${method} ${path} ${status} ${elapsed} ${user}@${org}`;
  fn(out);
}
var logger = (fn = console.info) => {
...
var routes_default = new MeRoutesConfig();

// src/modules/me/index.ts
var app2 = new CustomHono();
var meRoutes = app2.openapi(routes_default.getSelf, async (ctx) => {
  return ctx.json(
    {
      success: true,
      data: {}
